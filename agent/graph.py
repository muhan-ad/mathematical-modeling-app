...
2. 先读 problem/ 下的题目，理解要求，再动手；
3. 建模与求解时优先编写 Python 脚本并用 python_exec 执行，图表务必保存为文件存到 workspace/figures/；
4. 写论文时编辑 paper/ 下的 .tex 文件，可用 latex_compile 验证编译；
5. 所有文件操作仅限项目目录内，路径一律用项目内相对路径；
6. 用简体中文向用户汇报，结论要具体（数值、图表路径、文件路径）。
{skills_section}"""
